...
class DriverEarningsCubit extends BaseCubit<DriverEarningsState> {
  DriverEarningsCubit({
  ...
  void reset() {
    emit(const DriverEarningsState());
  }

  Future<void> getWallet() async {
    await taskManager.execute("DEC-gW1", () async {
      try {
        emit(
        ...
        logger.e(
          "[DriverEarningsCubit] - Error: ${e.message}",
          error: e,
          stackTrace: st,
        );
  ...
  Future<void> getTransactions() async {
    await taskManager.execute("DEC-gT1", () async {
      try {
        emit(
          state.copyWith(
  ...
  Future<void> getMonthlySummary({int? month, int? year}) async {
    await taskManager.execute("DEC-gMS1", () async {
      try {
        ...
        logger.e(
          "[DriverEarningsCubit] - Error getting monthly summary: ${e.message}",
          error: e,
          stackTrace: st,
        );
      }
