...
class UserLocationCubit extends BaseCubit<UserLocationState> {
  ...
  Future<Coordinate?> getMyLocation(
    ...
    logger.d('[UserLocationCubit] | getMyLocation called');
    try {
      ...
      logger.d('[UserLocationCubit] | getMyLocation result: $loc');
      if (loc != null) {
        final placemark = await _locationService.getPlacemark(
          lat: loc.y.toDouble(),
          lng: loc.x.toDouble(),
        );
        logger.d('[UserLocationCubit] | getPlacemark result: $placemark');
