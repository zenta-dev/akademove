...
import { ReportKeySchema } from "@repo/schema/report";
 ...
 *
 * @responsibility Generate WHERE clauses, count queries, and pagination logic for report listing
 */
export class ReportListQueryService {
	/**
	...
	/**
	 * Generate WHERE clauses for report listing
	 */
	static generateWhereClauses(options: {
		...
		if (search) {
			clauses.push(ilike(tables.report.description, `%${search}%`));
		}
	...
	/**
	 * Get count of reports matching search query
	 */
	static async getSearchCount(
		...
		try {
			const [dbResult] = await db
				...
				.from(tables.report)
				.where(ilike(tables.report.description, `%${search}%`));
