...
const { priv } = createORPCRouter(DriverWalletSpec);

export const DriverWalletHandler = priv.router({
	getWallet: priv.getWallet.handler(async ({ context, input: { params } }) => {
		...
		// IDOR Protection: Drivers can only access their own wallet
		...
		const res = await context.repo.wallet.getByUserId(driver.userId);
	...
	getTransactions: priv.getTransactions.handler(
		async ({ context, input: { params, query } }) => {
			...
			// IDOR Protection: Drivers can only access their own transactions
			if (context.user.role === "DRIVER" && driver.userId !== context.user.id) {
	...
	topUp: priv.topUp.handler(async ({ context, input: { params, body } }) => {
		...
		// IDOR Protection: Drivers can only top up their own wallet
		if (context.user.role === "DRIVER" && driver.userId !== context.user.id) {
	...
	transfer: priv.transfer.handler(
		async ({ context, input: { params, body } }) => {
			...
			// IDOR Protection: Drivers can only transfer from their own wallet
			if (context.user.role === "DRIVER" && driver.userId !== context.user.id) {
			...
			return await context.svc.db.transaction(async (tx) => {
				...
				if (driver.userId === data.recipientUserId) {
				...
				const senderUser = await tx.query.user.findFirst({
					where: (f, op) => op.eq(f.id, driver.userId),
				...
				logger.info(
					...
					"[DriverWalletHandler] Transfer completed",
				);
	...
	withdraw: priv.withdraw.handler(
		async ({ context, input: { params, body } }) => {
			...
			// IDOR Protection: Drivers can only withdraw from their own wallet
			if (context.user.role === "DRIVER" && driver.userId !== context.user.id) {
			...
			return await context.svc.db.transaction(async (tx) => {
				...
				// Save bank details to driver profile if requested
				if (data.saveBank) {
					const bankDetails = {
					...
					logger.info(
						{ userId: driver.userId, driverId: params.driverId },
						"[DriverWalletHandler] Saved bank details to driver profile",
					);
				}
				...
				const payoutResult = await context.svc.payout.createPayout({
					...
					amount: data.amount,
					notes: `AkadeMove driver withdrawal - ${transaction.id}`,
				});
				...
				logger.info(
					...
					"[DriverWalletHandler] Withdrawal payout initiated",
				);
	...
	getSavedBankAccount: priv.getSavedBankAccount.handler(
		async ({ context, input: { params } }) => {
			// Get driver to find userId and bank details
			const driver = await context.repo.driver.main.get(params.driverId);

			// IDOR Protection: Drivers can only access their own bank details
			if (context.user.role === "DRIVER" && driver.userId !== context.user.id) {
