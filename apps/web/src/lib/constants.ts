...
export const SUB_ROUTE_TITLES = Object.freeze({
	ADMIN: {
		...
		SETTINGS: `${m.configurations()} - ${ROUTE_TITLES.ADMINISTRATOR}`,
		...
		FRAUD: `Fraud Detection - ${ROUTE_TITLES.ADMINISTRATOR}`,
		COUPONS: `${m.coupons()} - ${ROUTE_TITLES.ADMINISTRATOR}`,
	} as const,
	OPERATOR: {
		...
		BANNERS: `${m.banners()} - ${ROUTE_TITLES.OPERATOR}`,
	...
	DRIVER: {
		...
		COMMISSION_REPORT: `${m.title_commission_report()} - ${ROUTE_TITLES.DRIVER}`,
