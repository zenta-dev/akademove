import 'package:test/test.dart';
import 'package:api_client/api_client.dart';

// tests for PricingConfiguration
void main() {
  final PricingConfiguration? instance = /* PricingConfiguration(...) */ null;
  // TODO add properties to the entity

  group(PricingConfiguration, () {
    // num baseFare
    test('to test the property `baseFare`', () async {
      // TODO
    });

    // num perKmRate
    test('to test the property `perKmRate`', () async {
      // TODO
    });

    // num minimumFare
    test('to test the property `minimumFare`', () async {
      // TODO
    });

    // num platformFeeRate
    test('to test the property `platformFeeRate`', () async {
      // TODO
    });

    // num taxRate
    test('to test the property `taxRate`', () async {
      // TODO
    });

    // num perKgRate
    test('to test the property `perKgRate`', () async {
      // TODO
    });

    // num merchantCommissionRate
    test('to test the property `merchantCommissionRate`', () async {
      // TODO
    });

  });
}
