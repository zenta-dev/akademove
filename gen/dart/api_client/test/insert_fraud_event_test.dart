import 'package:test/test.dart';
import 'package:api_client/api_client.dart';

// tests for InsertFraudEvent
void main() {
  final InsertFraudEvent? instance = /* InsertFraudEvent(...) */ null;
  // TODO add properties to the entity

  group(InsertFraudEvent, () {
    // FraudEventType eventType
    test('to test the property `eventType`', () async {
      // TODO
    });

    // FraudSeverity severity
    test('to test the property `severity`', () async {
      // TODO
    });

    // FraudStatus status
    test('to test the property `status`', () async {
      // TODO
    });

    // String userId
    test('to test the property `userId`', () async {
      // TODO
    });

    // String driverId
    test('to test the property `driverId`', () async {
      // TODO
    });

    // List<FraudSignal> signals
    test('to test the property `signals`', () async {
      // TODO
    });

    // num score
    test('to test the property `score`', () async {
      // TODO
    });

    // Location location
    test('to test the property `location`', () async {
      // TODO
    });

    // Location previousLocation
    test('to test the property `previousLocation`', () async {
      // TODO
    });

    // num distanceKm
    test('to test the property `distanceKm`', () async {
      // TODO
    });

    // int timeDeltaSeconds
    test('to test the property `timeDeltaSeconds`', () async {
      // TODO
    });

    // num velocityKmh
    test('to test the property `velocityKmh`', () async {
      // TODO
    });

    // String ipAddress
    test('to test the property `ipAddress`', () async {
      // TODO
    });

    // String userAgent
    test('to test the property `userAgent`', () async {
      // TODO
    });

    // String handledById
    test('to test the property `handledById`', () async {
      // TODO
    });

    // String resolution
    test('to test the property `resolution`', () async {
      // TODO
    });

    // String actionTaken
    test('to test the property `actionTaken`', () async {
      // TODO
    });

    // DateTime detectedAt
    test('to test the property `detectedAt`', () async {
      // TODO
    });

    // DateTime resolvedAt
    test('to test the property `resolvedAt`', () async {
      // TODO
    });

  });
}
